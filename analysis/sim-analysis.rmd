...
graph_tail(gci_normscheduler,  nogci_normscheduler, c("GCI", "NOGCI"),     title=tecdf_nsched,  x_limit_inf=40, x_limit_sup=70)
```
...
graph_tail(gci_opscheduler,   nogci_opscheduler,  c("GCI", "NOGCI"),    title=tecdf_opsched, x_limit_inf=40, x_limit_sup=70)
```

```{r  fig.width=15, fig.height=5, echo=FALSE, warning=FALSE}
tecdf_opgcisched = "FIGURA 3: ECDF RESPONSE TIME - OP GCI SCHEDULER"
graph_tail(gci_opgcischeduler, nogci_opscheduler, c("GCI", "NOGCI"), title=tecdf_opgcisched,  x_limit_inf=40, x_limit_sup=70)
```
