...
source("functions.R")
gci = filter(read.al(paste("../experiment/instrumented-exps/results-131kb-j8/", "gci1", ".csv", sep="")), status == 200)
nogci = filter(read.al(paste("../experiment/instrumented-exps/results-131kb-j8/", "nogci1", ".csv", sep="")), status == 200)
...
```
Neste gráfico é possível notar que a ECDF do GCI está dominando a ECDF do NOGCI.
...
```
O mesmo se aplica aqui. E note que os gráficos são bem parecidos o que pode nos indicar que não estamos tendo diferença significativa entre o tempo de resposta e o tempo de serviço. 
