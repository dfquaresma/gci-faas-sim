...
source("functions.R")
gci = filter(read.al(paste("../experiment/instrumented-exps/results-131kb-j8-copyimage-notbuffered/", "gci1", ".csv", sep="")), status == 200)
nogci = filter(read.al(paste("../experiment/instrumented-exps/results-131kb-j8-copyimage-notbuffered/", "nogci1", ".csv", sep="")), status == 200)
...
graph_tail(gci$response_time, nogci$response_time, title="THUMBNAILATOR ECDF RESPONSE TIME", x_limit_inf=0, x_limit_sup=200, annotate_y=0.90)
```

#### ECDF for service time
```{r  fig.width=15, fig.height=5, echo=FALSE}
graph_tail(gci$service_time, nogci$service_time, title="THUMBNAILATOR ECDF SERVICE TIME", x_limit_inf=0, x_limit_sup=200, annotate_y=0.90)
```
